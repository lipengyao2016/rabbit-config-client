...
class AmqpLibRecvClient extends AmqpBaseClient {
    ...
    setNackTime(nackTime)
    {
        this.nackRequeueTime = nackTime;
    }
    ...
    async createRecverConsumer(queueName,recvFunc, context,nackTime = 3000) {
        ...
        async function onRecvMsg(msg) {
            ...
            try
            {
                ...
                let ret = await  recvFunc.call(context, msg);
                if (ret) {
                    ctx.ack(msg);
                }
                else {
                    console.log('AmqpLibRecvClient recv msg dispatch  failed,msgValue: ' + msgValue);
                    ctx.reQueue(msg,ctx.nackRequeueTime);
                }
                /*               co.wrap(recvFunc).call(context, msg)
                                   ...
                                   console.log('AmqpLibRecvClient recv msg dispatch ,promise error:' + e);
                                   ctx.reQueue(msg, nackTime);
                                 })*/
            }
            catch(e)
